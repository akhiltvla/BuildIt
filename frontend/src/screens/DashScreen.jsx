...
const DashScreen = () => {
  ...
  const [projectsAssignedToSE, setProjectsAssignedToSE] = useState([]);

   const [progress, setProgress] = useState(0);

  const seProjects = async () => {
    try {
      // Use the project manager ID from pmInfo
      const siteEngineerName = userInfo ? userInfo.name : null;
      //  console.log('PM Name:',pmInfo.name);
      if (!siteEngineerName) {
        console.error('No Site Engineer found');
        return;
      }
    ...
    const projectsAssignedToSE = allProjects.filter(project => project.se === siteEngineerName);
    ...
    setProjectsAssignedToSE(projectsAssignedToSE);
  } catch (error) {
    console.error('Error fetching projects:', error);
          ...
          const handleView = (id) => {
            // console.log('ID:::',id);
            const project = projectsAssignedToSE.find((project) => project._id === id);
          ...
          const rows = projectsAssignedToSE && projectsAssignedToSE.map((list) =>
  ...
  return (
    ...
    <h3>
  Projects assigned to {userInfo.name}
